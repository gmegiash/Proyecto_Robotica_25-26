import torch
import torch.nn as nn
import torch.nn.functional as F
import torch.optim as optim
from torchvision import datasets, transforms
import os

# --- DEFINICIÓN DE LA RED (Simple y robusta) ---
class Net(nn.Module):
    def __init__(self):
        super(Net, self).__init__()
        self.conv1 = nn.Conv2d(1, 32, 3, 1)
        self.conv2 = nn.Conv2d(32, 64, 3, 1)
        self.dropout1 = nn.Dropout(0.25)
        self.dropout2 = nn.Dropout(0.5)
        self.fc1 = nn.Linear(9216, 128)
        self.fc2 = nn.Linear(128, 10)

    def forward(self, x):
        x = self.conv1(x)
        x = F.relu(x)
        x = self.conv2(x)
        x = F.relu(x)
        x = F.max_pool2d(x, 2)
        x = self.dropout1(x)
        x = torch.flatten(x, 1)
        x = self.fc1(x)
        x = F.relu(x)
        x = self.dropout2(x)
        x = self.fc2(x)
        output = F.log_softmax(x, dim=1)
        return output

def train():
    torch.manual_seed(42) # Para que siempre salga igual
    device = torch.device("cpu")
    
    # Transformaciones: Rotamos un poco para que aprenda que los números no siempre están rectos
    transform = transforms.Compose([
        transforms.RandomRotation(10),
        transforms.ToTensor(),
        # Normalización ESTÁNDAR de MNIST (Media 0.1307, Desviación 0.3081)
        transforms.Normalize((0.1307,), (0.3081,))
    ])

    print("⬇️ Descargando dataset MNIST...")
    dataset = datasets.MNIST('./data', train=True, download=True, transform=transform)
    train_loader = torch.utils.data.DataLoader(dataset, batch_size=64, shuffle=True)

    model = Net().to(device)
    optimizer = optim.Adam(model.parameters(), lr=0.001)

    print("🚀 Iniciando entrenamiento (3 Épocas)...")
    model.train()
    
    for epoch in range(1, 4):
        total_loss = 0
        correct = 0
        for batch_idx, (data, target) in enumerate(train_loader):
            data, target = data.to(device), target.to(device)
            optimizer.zero_grad()
            output = model(data)
            loss = F.nll_loss(output, target)
            loss.backward()
            optimizer.step()
            
            total_loss += loss.item()
            pred = output.argmax(dim=1, keepdim=True)
            correct += pred.eq(target.view_as(pred)).sum().item()
            
            if batch_idx % 100 == 0:
                print(f'Epoch {epoch} [{batch_idx*len(data)}/{len(train_loader.dataset)}] Loss: {loss.item():.4f}')

        acc = 100. * correct / len(train_loader.dataset)
        print(f"✅ Epoch {epoch} Finalizada. Precisión en entrenamiento: {acc:.2f}%")

    # GUARDADO
    save_path = "my_network.pt"
    torch.save(model.state_dict(), save_path)
    print(f"\n💾 Modelo guardado en: {os.path.abspath(save_path)}")
    print("⚠️  AHORA MUEVE ESTE ARCHIVO A TU CARPETA 'dnn' Y RENOMBRALO SI ES NECESARIO")

if __name__ == '__main__':
    train()